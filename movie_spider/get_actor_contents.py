...
def main():
    ...
    # 保存数据在json文件中
    save_path = r"workzone\movie_spider\actor_json" + "\\"
    save_actors_data(datalist, save_path)
...
def getUrl() -> list:
    ...
    从csv文件中读取要爬取网站的url
    '''
    # wb = xlrd.open_workbook(r"workzone\movie_spider\actors.xls")
    # sheet = wb.sheet_by_index(0)
    # urls = []
    # # 导入视频分割序列
    # for i in range(start, 14044):
    #     urls.append(str(sheet.cell_value(i, 0)))
    # linklist = urls
    # return urls
    urls = []
    with open(r"workzone\movie_spider\actors_3.csv", 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        raw_urls = []
        for row in reader:
            if(len(row) == 0):
...
# 简介
findIntro_1 = re.compile(r'<span class="all hidden">(.*?)</span\s*?>',re.S)
findIntro_2 = re.compile(r'</h2>\s*?</div>\s*?<div class="bd">(.*?)</div>\s*?</div>\s*?<div id="photos" class="mod">',re.S)
# 图片
findImgSrc = re.compile(r'<img alt=".*"\s*?title="点击看大图"\s*?src="(.*?)">')
# 性别
findSex = re.compile(r'<li>\s*?<span>性别</span>:\s*?(.*?)\s*?</li>')
# 星座
findStar = re.compile(r'<li>\s*?<span>星座</span>:\s*?(.*?)\s*?</li>')
# 出生日期
findBirth = re.compile(r'<li>\s*?<span>出生日期</span>:\s*?(.*?)\s*?</li>')
# 出生地
findBirthPlace = re.compile(r'<li>\s*?<span>出生地</span>:\s*?(.*?)\s*?</li>')
...
def getData(urls: list, browser):
    ...
    for raw_url in urls:
        ...
        url = "https://movie.douban.com" + raw_url
        # print(url)
        browser.get(url)
        ...
        # 逐一匹配简介
        intro = re.findall(findIntro_1,item)
        if(len(intro) == 0):
            intro = re.findall(findIntro_2,item)
        ...
        birthplace = re.findall(findBirthPlace,item)
        print(name)
        if(not name):
            continue
        if(len(intro) >= 1):
            intro[0] = re.sub(r'<br(\s+)?/?>(\s+)?'," ",intro[0])
            intro[0] = re.sub(r'\s+'," ",intro[0])
            # intro[0] = re.sub(r'\n'," ",intro[0])
            intro[0] = intro[0].strip()
        if(len(sex) >= 1):
            sex[0] = re.sub(r'<br(\s+)?/?>(\s+)?',"",sex[0])
            sex[0] = re.sub(r'\s+',"",sex[0])
            # intro[0] = re.sub(r'\n'," ",intro[0])
            sex[0] = sex[0].strip()
        if(len(star) >= 1):
            star[0] = re.sub(r'<br(\s+)?/?>(\s+)?',"",star[0])
            star[0] = re.sub(r'\s+',"",star[0])
            # intro[0] = re.sub(r'\n'," ",intro[0])
            star[0] = star[0].strip()
        if(len(birth) >= 1):
            birth[0] = re.sub(r'<br(\s+)?/?>(\s+)?',"",birth[0])
            birth[0] = re.sub(r'\s+',"",birth[0])
            # intro[0] = re.sub(r'\n'," ",intro[0])
            birth[0] = birth[0].strip()
        if(len(birthplace) >= 1):
            birthplace[0] = re.sub(r'<br(\s+)?/?>(\s+)?',"",birthplace[0])
            birthplace[0] = re.sub(r'\s+',"",birthplace[0])
            # intro[0] = re.sub(r'\n'," ",intro[0])
            birthplace[0] = birthplace[0].strip()
        movie_data.append(name)
        movie_data.append(intro)
        movie_data.append(picture)
        movie_data.append(sex)
        movie_data.append(star)
        movie_data.append(birth)
        movie_data.append(birthplace)
        movie_data.append(raw_url)
        # print(movie_data)
...
def save_actors_data(datalist, save_path):
    '''
    将电影的信息保存到json文件中
    '''
    for i in range(len(datalist)):
        file_path = save_path + ("%d.json"%(i+start - delta))
        with open(file_path,"w",encoding='utf-8') as f:
            ...
            if(len(datalist[i][1]) == 0):
                new_dict['intro'] = ''
            else:
                new_dict['intro'] = datalist[i][1][0]
            if(len(datalist[i][2]) == 0):
                new_dict['pictures_index'] = ''
            else:
                new_dict['pictures_index'] = datalist[i][2][0]
            if(len(datalist[i][3]) == 0):
                new_dict['sex'] = ''
            else:
                new_dict['sex'] = datalist[i][3][0]
            if(len(datalist[i][4]) == 0):
                new_dict['star'] = ''
            else:
                new_dict['star'] = datalist[i][4][0]
            if(len(datalist[i][5]) == 0):
                new_dict['birthday'] = ''
            else:
                new_dict['birthday'] = datalist[i][5][0]
            if(len(datalist[i][6]) == 0):
                new_dict['birthplace'] = ''
            else:
                new_dict['birthplace'] = datalist[i][6][0]
            new_dict['url'] = datalist[i][7]
